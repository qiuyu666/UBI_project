# -*- coding:utf-8 *-*
# @Time    : 2017/10/22 0022 17:17
# @Author  : LQY
# @File    : deaths_analyze.py
...
def deaths_statistic():  #统计各个等级的 流失 和 非流失 人数，并写进文件
    level_dic_0={}
    level_dic_1 = {}
    #count=0
    f=open('E:\8-22_Ubi_data\data_analyze\deaths_analyze\\for_28model_mean_deaths.csv','r')
    ...
    fw=open('E:\8-22_Ubi_data\data_analyze\deaths_analyze\\for_28model_deaths_dic_count_0.txt','w') #_1 表示非流失，改为_0表示流失人数文件
    fw1 = open('E:\8-22_Ubi_data\data_analyze\deaths_analyze\\for_28model_deaths_dic_count_1.txt', 'w')  # _1 表示非流失，改为_0表示流失人数文件
    for i in dict_0:
    ...
    #print count
#deaths_statistic()
def deaths_statistic_dis(): #将各个等级的 流失 和非流失 人数画图展示出来
    data1=[]
    data2=[]
    rate_of_users_0=[]
    rate_of_users_1 = []
    flag0=0
    flag1=0
    f= open('E:\8-22_Ubi_data\data_analyze\deaths_analyze\\for_7model_deaths_dic_count_0.txt','r')
    f1 = open('E:\8-22_Ubi_data\data_analyze\deaths_analyze\\for_7model_deaths_dic_count_1.txt','r')
    for line in f.readlines():
        line= line.split('\t')
        if flag0==1:
            data1.append(line[0])
    ...
    #x = range(data.)
    ax1=plt.subplot(121)
    plt.title(u'不同死亡数的流失人数-7') #图标名称
    #label = [u"流失", u"非流失"]  #标注
    plt.xlabel(u'平均死亡数')    #横坐标名称
    plt.ylabel(u'人数')    #纵坐标名称

    ax2 = plt.subplot(122)
    plt.title(u'不同死亡数的非流失人数-7')  # 图标名称
    ...
    fig_name = 'E:\8-22_Ubi_data\data_analyze\\deaths_analyze\\churn_count' + '\\' + 'churn_count_compare_7' + '.png'
    plt.savefig(fig_name)#要先保存再显示，因为显示完后会申请新的桌布，保存下来的就是空白，所以要先保存再显示
    plt.show()
deaths_statistic_dis()
